...
void
PMread(uint64_t physicalAddress, word_t* value)
{
  if (RAM.empty()) {
    initialize();
  }
...
void
PMwrite(uint64_t physicalAddress, word_t value)
{
  if (RAM.empty()) {
    initialize();
  }
...
void
PMevict(uint64_t frameIndex, uint64_t evictedPageIndex)
{
  if (RAM.empty()) {
    initialize();
  }
...
void
PMrestore(uint64_t frameIndex, uint64_t restoredPageIndex)
{
  if (RAM.empty()) {
    initialize();
  }
